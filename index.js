function Question (questionText, questionNo)
  ...
  const q1 = new Question("which of the method is used to get HTML element in javascript?", 1);
  const q2 = new Question("What does NaN means?", 2);
  const q3 = new Question("Which one of the following also known as Conditional Expression?", 3);
  ...
  const agetId = new Answer("getElementById");
  const aBoth = new Answer("Both"); 
  const aNone = new Answer("None of them");
  const qaPair1 = new QuestionAnswerPair(
    q1, [
      agetId, new Answer("getElementsByClassName()"),
      aBoth, aNone
    ], 
    agetId
  );
  
  const aNotaNumber = new Answer("Not a Number")
  const qaPair2 = new QuestionAnswerPair(
    q2, [
      new Answer("Negative Number"), aNotaNumber,
      aBoth, aNone
    ], 
    aNotaNumber
  );
  
  const aimmediateif = new Answer("Immediate If");
  const qaPair3 = new QuestionAnswerPair(
    q3, [
      new Answer("Alternative to if-else"), new Answer("Switch statement"),
      new Answer("If-then-else statement"), aimmediateif
    ], 
    aimmediateif
  );
  ...
  const quizApp = new QuizApplication([qaPair1, qaPair2, qaPair3, qaPair4, qaPair5]);
  quizApp.start();